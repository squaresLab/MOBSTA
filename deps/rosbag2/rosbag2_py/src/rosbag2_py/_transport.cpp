...
// https://github.com/ros2/rosbag2/blob/0.15.3/rosbag2_py/src/rosbag2_py/_transport.cpp commit 2579e9f
//
...
namespace rosbag2_py
{
  ...
  class MutationPlayer
  {
  public:
    /**
     * @brief constructor that creates a ros node and provides an interface for python bindings.
     */
    MutationPlayer()
    {
      rclcpp::init(0, nullptr);
    ...
    /**
     * @brief calls the mutation player class from the rosbag2_transport library.
     *
     * @param storage_options infromation about the bag and how to store it.
     * @param play_options options for playing the bag provided as command line arguments.
     * @param mutation_config_file file name of the JSON file of the mutation
     *    configuration with the topic names and time ranges where mutations are applied.
     */
    void play(
        const rosbag2_storage::StorageOptions &storage_options,
        PlayOptions &play_options, const std::string &mutation_config_file)
    {
      auto reader = rosbag2_transport::ReaderWriterFactory::make_reader(storage_options);
      auto player = std::make_shared<rosbag2_transport::MutationPlayer>(
          std::move(reader), storage_options, play_options, mutation_config_file);
...
PYBIND11_MODULE(_transport, m)
{
  ...
  py::class_<PlayOptions>(m, "PlayOptions")
      ...
      .def_readwrite("seed", &PlayOptions::seed);
  ...
  py::class_<rosbag2_py::MutationPlayer>(m, "MutationPlayer")
      .def(py::init())
      .def("play", &rosbag2_py::MutationPlayer::play);
