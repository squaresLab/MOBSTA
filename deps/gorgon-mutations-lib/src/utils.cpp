...
#define ADD_ReplaceWholeArray(primitiveType, primitiveTypeString) \
    ...
    [](json args, unsigned long seed) -> std::shared_ptr<BaseMutator> { \
        return std::make_shared<FixedSizeNumericArray::ReplaceWholeArrayMutator<primitiveType>>(args.at("replacementVal").get<primitiveType>(), \
...
#define ADD_ProbabalisticReplace(primitiveType, primitiveTypeString) \
    ...
    [](json args, unsigned long seed) -> std::shared_ptr<BaseMutator> { \
        return std::make_shared<FixedSizeNumericArray::ProbabalisticReplaceMutator<primitiveType>>(args.at("replacementValue").get<primitiveType>(), \
...
#define ADD_AddToWholeArray(primitiveType, primitiveTypeString) \
    ...
    [](json args, unsigned long seed) -> std::shared_ptr<BaseMutator> { \
        return std::make_shared<FixedSizeNumericArray::AddToWholeArrayMutator<primitiveType>>(args.at("valueToAdd").get<primitiveType>(), \
                                                                                              args.at("minArrayValue").get<primitiveType>(), \
                                                                                              args.at("maxArrayValue").get<primitiveType>(), \
                                                                                              args.at("arraySize").get<std::size_t>()); \
    };

#define ADD_ARRAY_MUTATORS_FOR_ALL_TYPES(mutatorType) \
...
#define ADD_GaussianNoiseMutator(primitiveType, primitiveTypeString) \
    m_MutatorMap[std::string(primitiveTypeString) + "_GaussianNoiseMutator"] = \
    [](json args, unsigned long seed) -> std::shared_ptr<BaseMutator> { \
        return std::make_shared<Numeric::GaussianNoiseMutator<primitiveType>>(args.at("mean").get<double>(), \
                                                                                   args.at("stdDev").get<double>(), \
                                                                                   seed);\
    };

#define ADD_StuckValueMutator(primitiveType, primitiveTypeString) \
    m_MutatorMap[std::string(primitiveTypeString) + "_StuckValueMutator"] = \
    [](json args, unsigned long seed) -> std::shared_ptr<BaseMutator> { \
        return std::make_shared<Numeric::StuckValueMutator<primitiveType>>(args.at("stuckValue").get<primitiveType>());\
    };

#define ADD_SINGLE_NUMBER_MUTATORS_FOR_ALL_TYPES(mutatorType) \
    ADD_##mutatorType(double, "Double") \
    ADD_##mutatorType(float, "Float32") \
    ADD_##mutatorType(int, "Int32") \
    ADD_##mutatorType(unsigned int, "UInt32") \
...
utils::MutatorFactory::MutatorFactory()
{
    ...
    ADD_ARRAY_MUTATORS_FOR_ALL_TYPES(ReplaceWholeArray)
    ADD_ARRAY_MUTATORS_FOR_ALL_TYPES(ProbabalisticReplace)
    ADD_ARRAY_MUTATORS_FOR_ALL_TYPES(AddToWholeArray)

    ADD_SINGLE_NUMBER_MUTATORS_FOR_ALL_TYPES(GaussianNoiseMutator)
    ADD_SINGLE_NUMBER_MUTATORS_FOR_ALL_TYPES(StuckValueMutator)
}
