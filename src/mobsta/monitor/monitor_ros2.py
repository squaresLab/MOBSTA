...
class MonitorROS2(Node):
    ...
    Attributes:
        monitor_report (dict): monitor violations report where each invariant
            name with a list of timestamps for violations
    ...
    def __init__(self, config_file, test_dir, metadata_file):  # noqa: D107
        ...
        self.create_subscription(KeyValue, '/violations', self.callback, 10)

        MonitorROS2.monitor_report['SUT Crash'] = {
            'violations': [],
            'config': {}
        }
        ...
        for invariant in config_json:
            ...
            MonitorROS2.monitor_report[invariant_name] = {
                'violations': [],
                'config': invariant
            }
    ...
    def callback(self, msg):
        ...
        """
        MonitorROS2.monitor_report[msg.key]['violations'].append(msg.value)

    def handle_sigcrash(self, sig, frame):
        """Handles the Signal triggered by the SUT failure and pass it to the invariant node,
        Then shutdown rclpy node and exit the program
        ...
        MonitorROS2.monitor_report["SUT Crash"]['violations'].append(duration)
    ...
    def get_json_report(self):
        ...
        """
        results = []
        for inv_name, inv_data in MonitorROS2.monitor_report.items():
            result = {
    ...
    def clean_up(self):
        ...
        for node in self.invariant_nodes.values():
            ...
            if ret is not None:
                ...
                print(node_stderr, file=sys.stderr)
...
def main(args):
    ...
    monitor = MonitorROS2(args.config_file, args.test_dir, args.metadata_file)
    signal.signal(signal.SIGUSR1, monitor.handle_sigcrash)
    rclpy.get_default_context().on_shutdown(monitor.close)
    invariant_status_check_timer = monitor.create_timer(INVARIANT_CHECK, monitor.check_invariants_running)

    try:
        rclpy.spin(monitor)
    # We're expecting external shutdown, so just mask that
    except (rclpy.executors.ExternalShutdownException):
        monitor.get_json_report()
        monitor.clean_up()
        pass
...
def callable_path():
    ...
    """
    return os.path.join(os.path.dirname(os.path.abspath(__file__)), "monitor_ros2.py")
