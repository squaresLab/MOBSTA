...
class CorrectCommandsInvariantROS2(invariant_base_ros2.BaseInvariantROS2):
    """
    ...
    def __init__(self):
        super(CorrectCommandsInvariantROS2, self).__init__()
        self.params = self.get_params()
              
        self.create_subscription(geometry_msgs.msg.Twist, self.params["velocity_topic"], self.velocityCallback, 10)
        self.create_subscription(sensor_msgs.msg.JointState, self.params["joint_states_topic"], self.jointStatesCallback, 10)
...
def main():
    rclpy.init()
    correct_command_inv = CorrectCommandsInvariantROS2()

    try:
        rclpy.spin(correct_command_inv)
    except KeyboardInterrupt:
        pass

    correct_command_inv.destroy_node()
