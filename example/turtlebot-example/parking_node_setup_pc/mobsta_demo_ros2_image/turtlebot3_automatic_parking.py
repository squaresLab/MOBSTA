    ...
    def park_robot(self):
        ...
        if self.step == 0:
            if scan_done:
                if fining_spot:
                    ...
                    self.get_logger().info('| start  | {0:>10.3f}| {1:>10.3f}|'.format(start_point[0], start_point[1]))
                    self.get_logger().info('| center | {0:>10.3f}| {1:>10.3f}|'.format(self.center_point[0], self.center_point[1]))
                    self.get_logger().info('| end    | {0:>10.3f}| {1:>10.3f}|'.format(end_point[0], end_point[1]))
                    self.get_logger().info("=================================")
                    self.get_logger().info('| theta  | {0:.2f} deg'.format(numpy.rad2deg(self.theta)))
                    self.get_logger().info('| yaw    | {0:.2f} deg'.format(numpy.rad2deg(self.yaw)))
                    self.get_logger().info("=================================")
                    self.get_logger().info("===== Rotate to parking spot!!! =====")
                    self.step = 1
            else:
                self.get_logger().info("Fail finding parking spot.")

        # Step 1: Turn
        elif self.step == 1:
            if self.theta < numpy.deg2rad(self.angle_tolerance_deg) or self.theta > numpy.deg2rad(180-self.angle_tolerance_deg):
                self.get_logger().info("===== Move to parking spot!!!! =====")
                twist.angular.z = 0.0
        ...
        # Step 2: Move straight
            ...
            if self.center_point[0] < -self.position_tolerance:
                ...
                twist.linear.x = 0.0
                twist.angular.z = 0.0
                self.get_logger().info("===== Rotate to line up!!!!! =====")
                self.step = 3
        ...
        # Step 3: Turn
            ...
            if self.theta > numpy.deg2rad(90 - self.angle_tolerance_deg) and self.theta < numpy.deg2rad(90 + self.angle_tolerance_deg):
                twist.linear.x = 0.0
                twist.angular.z = 0.0
                self.get_logger().info("===== Get closer!!!!!! =====")
                self.step = 4
        ...
        # Step 4: Move Straight
            ...
            if len(ranges) > 0:
                if min(ranges) > self.distance_tolerance:
                    ...
                    self.get_logger().info("Automatic parking done.")
                    self.cmd_vel_pub.publish(twist)
                    self.step = -1
